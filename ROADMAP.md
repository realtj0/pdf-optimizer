...
- 2026-07-01 — Investigated moving it from "Quick Actions ▸" to the main context menu (like Skim/CotEditor's items). Conclusion, decided not worth pursuing: top-level promotion (`FinderActive`/`FinderOrdering` in `pbs`) is only available to Apple's built-in Finder extensions and Shortcuts.app quick actions — never Automator `.workflow` services, regardless of registration. Skim/CotEditor achieve it by registering native `NSServices` from compiled app code (a different, lower-level API, not reachable via Automator or Shortcuts). Re-saving the workflow through Automator itself (not just hand-editing the plist) was still necessary and sufficient to get proper "Quick Actions ▸" registration. Staying at "Quick Actions ▸ Optimize PDF" — good enough, not worth an Xcode project for one fewer click.
